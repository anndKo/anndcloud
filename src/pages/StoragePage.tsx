...
import { Cloud, Camera, Image, File, Link as LinkIcon, LogOut, RefreshCw, Loader2, Search, X, MoreVertical, CheckSquare, Trash2, Share2, Download, Play, Eye, Maximize2 } from 'lucide-react';
...
export default function StoragePage() {
  ...
  return <div className="min-h-screen gradient-bg">
        ...
        <div className="container mx-auto px-4 py-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              {selectionMode ? <Button variant="ghost" size="icon" onClick={exitSelectionMode} className="rounded-full">
                  <X className="w-5 h-5" />
                </Button> : <>
      ...
      <main className="container mx-auto px-4 py-6 space-y-6">
        ...
        delay: 0.2
        ...
        }} className={`tab-item ${activeTab === 'media' ? 'tab-item-active' : ''}`}>
            <Image className="w-4 h-4 inline mr-2" />
            Ảnh & Video
            {mediaFiles.length > 0 && <span className="ml-2 text-xs bg-primary/20 text-primary px-2 py-0.5 rounded-full">
                {mediaFiles.length}
              </span>}
          </button>
          <button onClick={() => {
          setActiveTab('files');
          if (selectionMode) setSelectedItems(new Set());
        }} className={`tab-item ${activeTab === 'files' ? 'tab-item-active' : ''}`}>
            <File className="w-4 h-4 inline mr-2" />
            File
            {otherFiles.length > 0 && <span className="ml-2 text-xs bg-primary/20 text-primary px-2 py-0.5 rounded-full">
                {otherFiles.length}
              </span>}
          </button>
          <button onClick={() => {
          setActiveTab('links');
          if (selectionMode) setSelectedItems(new Set());
        }} className={`tab-item ${activeTab === 'links' ? 'tab-item-active' : ''}`}>
            <LinkIcon className="w-4 h-4 inline mr-2" />
            Link
            {filteredLinks.length > 0 && <span className="ml-2 text-xs bg-primary/20 text-primary px-2 py-0.5 rounded-full">
          ...
          {loading ? <div className="flex items-center justify-center py-20">
            ...
            x: 20
                ...
                y: 0
                      ...
                      <MediaGrid items={mediaFiles} onUpdate={fetchData} selectionMode={selectionMode} selectedItems={selectedItems} onSelectionChange={handleSelectionChange} />
                    </>}
            ...
            x: -20
                ...
                </motion.div> : <motion.div key="links" initial={{
            opacity: 0,
            x: 20
          }} animate={{
            opacity: 1,
